...
if __name__ == '__main__':
    from model_drop import CharRNN, CharLSTM
    ...
    model_rnn.load_state_dict(torch.load('best_rnn_model_drop.pth'))
    model_rnn.eval()
    ...
    for i, seed_chars in enumerate(seed_chars_list):
        ...
        with open(f'drop_generated_text_rnn_sample_{i+1}.txt', 'w') as f:
    ...
    model_lstm.load_state_dict(torch.load('best_lstm_model_drop.pth'))
    model_lstm.eval()

    # 5개의 LSTM 샘플 생성
    for i, seed_chars in enumerate(seed_chars_list):
        generated_text_lstm = generate(model_lstm, seed_chars, temperature, char_to_idx, idx_to_char, device, length, is_lstm=True)
        with open(f'drop_generated_text_lstm_sample_{i+1}.txt', 'w') as f:
            # f.write("seed_character : " + seed_chars)            
